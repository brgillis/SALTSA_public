#ifndef _SPLINE_INCLUDED_
#define _SPLINE_INCLUDED_
...
/* dynamo:- Event driven molecular dynamics simulator
 ...
 See https://www.marcusbannerman.co.uk/index.php/home/latestarticles/42-articles/96-cubic-spline-class.html
 */
...
public:
	...
	//end points
	Spline() :
			_valid(false), _BCLow(FIXED_1ST_DERIV_BC), _BCHigh(
					FIXED_1ST_DERIV_BC), _BCLowVal(0), _BCHighVal(0), _type(
					CUBIC) {
	}
	...
	std::pair<double, double> at(unsigned int i) const {
		return base::at(i);
	}
...
private:
	...
	inline double highCalc(double xval) {
		...
		const double firstDeriv = 2 * h(size() - 2)
				* (_ddy[size() - 2] + 2 * _ddy[size() - 1]) / 6
...
//just slightly seperate them
		{
			...
			while (!testPassed) {
				...
				for (base::iterator iPtr = base::begin();
						iPtr != base::end() - 1; ++iPtr)
