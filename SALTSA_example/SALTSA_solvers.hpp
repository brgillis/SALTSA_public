		...
		const int max_steps = 10000, const bool silent = false )
{
	...
	if ( in_params_to_use.size() < (unsigned)num_in_params )
	{
		if ( !in_params_to_use.empty() )
		{
			if ( !silent )
				std::cerr
						<< "WARNING: Size mismatch of num_in_params and in_params_to_use.size() in SALTSA::unit_solve_sd.\n";
		}
		...
		const bool silent = false)
{
	...
	{
		// Get a new value
		test_in_param = current_in_param + in_param_step_sigma*SALTSA::Gaus_rand()/annealing;

		// Check if it's in bounds
		if(bounds_check)
			test_in_param = SALTSA::bound(min_in_param,test_in_param,max_in_param);
		...
		if(SALTSA::divisible(annealing_period,step))
		{
		...
		const bool silent = false)
{
	...
	double last_log_likelihood = SALTSA::sum(
			SALTSA::multiply(-annealing/2,out_params));

	for(int step = 0; step < max_steps; step++)
	{
		// Get a new value
		test_in_params = SALTSA::rand_vector(SALTSA::Gaus_rand,
				                               current_in_params,
				                               SALTSA::divide(in_param_step_sigmas,annealing));

		// Check if it's in bounds
		if(bounds_check)
		{
			test_in_params = SALTSA::apply(SALTSA::min<T>,test_in_params,max_in_params);
			test_in_params = SALTSA::apply(SALTSA::max<T>,test_in_params,min_in_params);
		}
		...
		double new_log_likelihood = SALTSA::sum(
				SALTSA::multiply(-annealing/2,out_params));
		...
		if(good_result)
		{
			...
			if(step_to_it)
			{
				...
				// Check if we have a new best point
				if(SALTSA::sum(out_params) < SALTSA::sum(best_out_params))
				{
					best_out_params = out_params;
		...
		if(last_cycle)
		{
			mean_in_params = SALTSA::add(mean_in_params,current_in_params);
			last_cycle_count += 1;
		}

		if((step!=0)&&(SALTSA::divisible(step,annealing_period)))
		{
			annealing *= annealing_factor;
	...
	// Calculate mean now
	mean_in_params = SALTSA::divide(mean_in_params,SALTSA::safe_d(last_cycle_count));
