...
#define DEFINE_BRG_CACHE_STATIC_VARS(class_name,init_min,init_max,init_step) \
	double SALTSA::class_name::_mins_ = init_min;							\
	double SALTSA::class_name::_maxes_ = init_max; 						\
	double SALTSA::class_name::_steps_ = init_step; 						\
	bool SALTSA::class_name::_loaded_ = false;							\
	bool SALTSA::class_name::_initialised_ = false;						\
	short int SALTSA::class_name::_is_monotonic_ = 0;						\
	unsigned int SALTSA::class_name::_sig_digits_ = 8;					\
	unsigned int SALTSA::class_name::_resolutions_ = 0;					\
	std::string SALTSA::class_name::_file_name_ = "";						\
	std::string SALTSA::class_name::_header_string_ = "";					\
	std::vector<double> SALTSA::class_name::_results_;
	...
	const int _load( const bool silent = false ) const
	{
		...
		do
		{
			if ( loop_counter >= 2 )
			{
				if ( !silent )
					std::cerr
							<< "ERROR: infinite loop detected trying to load " + _file_name_ + " in SALTSA::brg_cache.\n";
...
template<typename name>
double SALTSA::brg_cache<name>::_mins_ = 0;

template<typename name>
double SALTSA::brg_cache<name>::_maxes_ = 0;

template<typename name>
double SALTSA::brg_cache<name>::_steps_ = 1; // To avoid divide-by-zero issues

// These ones don't need to be altered for each child, but they should be copied
template<typename name>
bool SALTSA::brg_cache<name>::_loaded_ = false;

template<typename name>
bool SALTSA::brg_cache<name>::_initialised_ = false;

template<typename name>
short int SALTSA::brg_cache<name>::_is_monotonic_ = 0;

template<typename name>
unsigned int SALTSA::brg_cache<name>::_sig_digits_ = 8;

template<typename name>
unsigned int SALTSA::brg_cache<name>::_resolutions_ = 0;

template<typename name>
std::string SALTSA::brg_cache<name>::_file_name_ = "";

template<typename name>
std::string SALTSA::brg_cache<name>::_header_string_ = "";

template<typename name>
std::vector<double> SALTSA::brg_cache<name>::_results_;
...
#endif // __SALTSA_CACHE_HPP_INCLUDED__
