...
#if (1)
double brgastro::spline_derivative::_default_sample_scale_ = 0.02; // As a fraction of t_max-t_min
double brgastro::spline_derivative::_default_sample_max_width_ = 0.05; // As a fraction of t_max-t_min
double brgastro::spline_derivative::_default_sample_precision_ = 0.01;
#endif
...
double brgastro::stripping_orbit::_default_step_length_power_ = 1.5;
double brgastro::stripping_orbit::_default_step_factor_max_ = 10; // Maximum allowed value of (v_0/v)^(step_length_power)
double brgastro::stripping_orbit::_default_step_factor_min_ = 0.01; // Minimum allowed value of (v_0/v)^(step_length_power)
#endif
...
#if (1)
// brgastro::spline_function class method implementations
#if (1)
// Constructors
...
// Function method
const int brgastro::spline_function::operator()( const BRG_UNITS & in_param,
...
// brgastro::spline_derivative_function class method implementations
#if (1)
...
// Function method
const int brgastro::spline_derivative_function::operator()(
...
// brgastro::spline_derivative_weight_function class method implementations
#if (1)
// Constructors
brgastro::spline_derivative_weight_function::spline_derivative_weight_function()
{
	_sample_scale_ = 0;
...
// Set functions
const int brgastro::spline_derivative_weight_function::set_sample_scale(
...
const int brgastro::spline_derivative_weight_function::set_sample_max_width(
...
const int brgastro::spline_derivative_weight_function::set_center_point(
...
const int brgastro::spline_derivative_weight_function::set_t_min(
		double new_t_min )
{
	_t_min_ = new_t_min;
	return 0;
...
const int brgastro::spline_derivative_weight_function::set_t_max(
		double new_t_max )
{
	_t_max_ = new_t_max;
	return 0;
...
// Function method
const int brgastro::spline_derivative_weight_function::operator()(
		const BRG_UNITS & in_param,
		BRG_UNITS & out_param, const bool silent ) const
{
...
// brgastro::spline_derivative class method implementations
#if (1)
// Constructors
...
const int brgastro::spline_derivative::clear_spline_ptr()
{
...
const int brgastro::spline_derivative::set_default_sample_scale(
...
const int brgastro::spline_derivative::set_default_sample_max_width(
...
const int brgastro::spline_derivative::set_sample_scale(
		double new_sample_scale )
{
	_sample_scale_ = new_sample_scale;
	_calculated_ = false;
...
const int brgastro::spline_derivative::set_sample_max_width(
		double new_sample_max_width )
{
	_sample_max_width_ = new_sample_max_width;
	_calculated_ = false;
...
const int brgastro::spline_derivative::reset_sample_scale() // Sets it to default
{
...
const int brgastro::spline_derivative::reset_sample_max_width() // Sets it to default
{
...
const int brgastro::spline_derivative::clear_known_points()
{
...
const int brgastro::spline_derivative::clear_unknown_points()
{
...
const int brgastro::spline_derivative::clear_points()
{
	_known_spline_.clear();
	_unknown_t_list_.clear();
	_calculated_ = false;
...
const int brgastro::spline_derivative::clear()
{
	_spline_ptr_ = 0;
	_spline_ptr_set_up_ = false;
	_known_spline_.clear();
...
const int brgastro::spline_derivative::add_unknown_point( const double t )
{
...
// Get functions
const double brgastro::spline_derivative::operator()( double xval ) const
{
	if ( !_spline_ptr_set_up_ )
	{
...
const int brgastro::stripping_orbit::_init()
{
	return clear();
}
		...
		const stripping_orbit &other_stripping_orbit )
{
	...
	else
	{
		...
		std::vector< brgastro::stripping_orbit_segment >::iterator test_iterator_old = other_stripping_orbit._orbit_segments_.begin();
...
const int brgastro::stripping_orbit_segment::calc( const bool silent ) const
{
	...
	_rt_list_.push_back(
			brgastro::get_rt( _current_host_ptr_, _current_satellite_ptr_, r,
			...
			t += t_step * step_length_factor )
	{
		...
		try
		{
			...
			mret = brgastro::tidal_strip_retained( _current_host_ptr_,
			...
			double gabdt_scaling_factor = 1
					- ( t_step * step_length_factor
							/ ( t_recover * tidal_shocking_persistance ) );
			...
			_rt_list_.push_back(
					brgastro::get_rt( _current_host_ptr_,
							_current_satellite_ptr_, r, vr, vt,
							t_step * step_length_factor,
							_sum_delta_rho_list_.at( counter - 1 ) ) );
		...
		const bool silent ) const
{
	if ( index < 1 )
		return 0;
	double result = ( 1. / 6. / Gc * std::pow( 1 + x, tidal_shocking_power )
			* tidal_shocking_amplification
...
const double brgastro::stripping_orbit_segment::_step_length_factor( const BRG_VELOCITY & v, const BRG_DISTANCE & r ) const
{
	...
	double step_length_factor_v = bound( step_factor_min,
			std::pow( std::fabs(v_0 / safe_d( v )), step_length_power ), step_factor_max);

	// Low-r -> small step factor (and the inverse), bounded by max and min
	double step_length_factor_r = bound( step_factor_min,
			std::pow( std::fabs(r / safe_d( r_0 )), step_length_power ), step_factor_max);
...
// brgastro::solve_rt_sd_function class method implementations
#if (1)

const int brgastro::solve_rt_grid_function::operator()(
		const BRG_UNITS & in_param,
		BRG_UNITS & out_param, const bool silent ) const
{
	BRG_DISTANCE r;
...
const int brgastro::solve_rt_it_function::operator()(
		const BRG_UNITS & in_param,
		BRG_UNITS & out_param, const bool silent ) const
{
	BRG_DISTANCE r = 0;
...
#endif // end brgastro::solve_rt_sd_function class function definitions
...
brgastro::gabdt::gabdt( const gabdt & other_gabdt )
{
...
// brgastro::gabdt_function class method implementations
#if (1)

brgastro::gabdt_function::gabdt_function()
{
	host_ptr = 0;
}

const int brgastro::gabdt_function::operator()(
...
#endif // end brgastro::gabdt_function class function definitions
...
const BRG_DISTANCE brgastro::get_rt( const density_profile *host_group,
		const density_profile *satellite, const BRG_DISTANCE &r,
		const BRG_VELOCITY &vr, const BRG_VELOCITY &vt,
		const BRG_TIME &time_step, const BRG_UNITS &sum_rho,
		const bool silent )
